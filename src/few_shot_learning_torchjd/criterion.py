...
class RegularizedLoss(nn.Module):
    def __init__(self, 
        ...
        self.non_negative_projection = non_negative_projection
    ...
    def _non_negative_params_loss(self, model: nn.Module) -> torch.Tensor:
        pass
...
class BinaryRegularizationLoss(nn.Module):
    def __init__(self, 
                 l1_weight: float,
                 l2_weight: float,
                 cross_entropy_weight: float,
                 binary_reg_weight: float):
        super().__init__()
        self.l1_weight = l1_weight
        self.l2_weight = l2_weight
        self.bce_weight = cross_entropy_weight
        self.binary_reg_weight = binary_reg_weight


    def forward(self, model: nn.Module, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:

        output = model(input)
        bce_loss = F.cross_entropy(output, target, reduction='none').mean(dim=0, keepdim=True)
        ...
        if self.l2_weight > 0:
            l2_loss = sum(p.pow(2).mean() for p in model.parameters())
        else:
            l2_loss = 0

        if self.binary_reg_weight > 0:
            def binary_reg(x):
                return F.relu(x+1) - F.relu(x) - F.relu(-x) + F.relu(-x+1) + F.relu(x-1) + F.relu(-1-x)
            binary_reg_loss = sum(binary_reg(p).sum() for p in model.parameters())
        else:
            binary_reg_loss = torch.zeros([])

        # total_loss = self.bce_weight * bce_loss + \
        #                 self.l1_weight * l1_loss + \
        #                 self.l2_weight * l2_loss + \
        #                 self.binary_reg_weight * binary_reg_loss
        #return total_loss
        return torch.cat([self.bce_weight * bce_loss, self.binary_reg_weight * binary_reg_loss.unsqueeze(0)])
